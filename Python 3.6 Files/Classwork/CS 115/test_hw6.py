...
class Test(unittest.TestCase):
    ...
    def test07(self):
        sequence = '0110' * 16
        compress = hw6.compress(sequence)
        self.assertEqual(compress, '000010001000010000100001000010000100001000010000100001000010000100001000010000100001000010000100001000010000100001000010000100001000010000100001000010000100001000001')
        uncompress = hw6.uncompress(compress)
        self.assertEqual(uncompress, sequence)
        self.assertAlmostEqual(hw6.compression(sequence), 2.578125, 4)
        
    def test08(self):
        sequence = '1' * (hw6.MAX_RUN_LENGTH + 1) + '0' * (hw6.MAX_RUN_LENGTH + 1) + '1' * (64 - 2 * hw6.MAX_RUN_LENGTH - 2)
        compress = hw6.compress(sequence)
        self.assertEqual(compress, '00000111110000000001111110000000001')
        uncompress = hw6.uncompress(compress)
        self.assertEqual(uncompress, sequence)
        self.assertAlmostEqual(hw6.compression(sequence), 0.546875, 4)
