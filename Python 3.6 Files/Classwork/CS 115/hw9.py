...
def questifyAlt(mylist):
    '''Same as questify'''
    return map(lambda x: x + '?',mylist)
...
def wordScoreLoop(myword, scorelist):
    '''Same as wordScore'''
    finalscore = 0
    for letter in myword:
        finalscore += letterScoreLoop(letter,scorelist)
    return finalscore
...
def wordsWithScoreLoop(dct, scores):
    ...
    finallist = []
    for index in dct:
        finallist.append([index,wordScoreLoop(index,scores)])
    return finallist
